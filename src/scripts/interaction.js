 ...
 *
 * @class H5P.BrightcoveInteractiveVideoInteraction
 * @extends H5P.EventDispatcher
 * @param {Parameters} parameters describes action behavior
 * @param {H5P.BrightcoveInteractiveVideo} player instance
...
function Interaction(parameters, player, previousState) {
  ...
  var createButton = function (preventAnimation) {
    ...
    if (self.getRequiresCompletion() &&
        ...
        player.currentState !== H5P.BrightcoveInteractiveVideo.SEEKING) {
      openDialog(true);
    }
   ...
   *
   * @return {Array.<H5P.BrightcoveInteractiveVideoInteraction>}
  ...
  self.getCopyrights = function () {
    if (!self.isStandaloneLabel()) {
      ...
      if (instance !== undefined) {
        ...
        interactionCopyrights.setLabel(title + ' ' + H5P.BrightcoveInteractiveVideo.humanizeTime(parameters.duration.from) + ' - ' + H5P.BrightcoveInteractiveVideo.humanizeTime(parameters.duration.to));
  ...
  self.getClipboardData = function () {
    return H5P.DragNBar.clipboardify(H5PEditor.BrightcoveInteractiveVideo.clipboardKey, parameters, 'action');
  };
